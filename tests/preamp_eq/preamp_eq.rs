...
// Command: wdf_compiler preamp_eq.wdf preamp_eq.rs -lang rust

#[derive(Default,Copy,Clone)]
pub struct Params {
    pub lfc_cap_value : f32, // = 4.7e-09
    pub r_series_value : f32, // = 1.0e+04
    pub hfl_res_value : f32, // = 8.0e+04
    pub hfl_cap_value : f32, // = 2.7e-09
    pub hfc_cap_value : f32, // = 3.3e-06
    pub r_shunt_value : f32, // = 4.674e+03
    pub lfl_res_value : f32, // = 1.5e+04
    pub lfl_cap_value : f32, // = 1.5e-07
}
...
pub fn calc_impedances(impedances: &mut Impedances, fs: f32, params: Params) {
    ...
    let hfl_res_cap_res_value = params.hfl_res_value;
    let hfl_res_cap_cap_value = params.hfl_cap_value;
    ...
    let lfl_res_cap_res_value = params.lfl_res_value;
    let lfl_res_cap_cap_value = params.lfl_cap_value;
    ...
    // Computing impedance for: v_in;
    let v_in_cap_value = params.lfc_cap_value;
    impedances.v_in_ca = T / (T + 2.0 * s_o_R * v_in_cap_value);
    impedances.v_in_ce = (2.0 * s_o_R * v_in_cap_value) / (T + 2.0 * s_o_R * v_in_cap_value);

}

#[allow(unused)]
pub fn process(state: &mut State, impedances: &Impedances, v_in: f32) -> f32 {
    ...
    let v_in_a : f32 = s_o_b; // v_in incident
    let v_in_a0a1 : f32 = 2.0 * (v_in + state.v_in_z) + v_in_a; // v_in reflected
    let v_in_b : f32 = impedances.v_in_ca * v_in_a - impedances.v_in_ce * v_in_a0a1; // v_in reflected
    state.v_in_z += -v_in_b - v_in_a0a1; // v_in_z state update
    let s_o_a : f32 = v_in_b; // s_o incident
