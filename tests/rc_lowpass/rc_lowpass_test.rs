...
fn main() {
    println!("RC Lowpass test");

    let ref_output = get_data(std::path::Path::new("data.bin"));

    let fs : f32 = 48000.0;

    let params = rc_lowpass::Params { R1_value: 1.0e+03, C1_value: 1.0e-06 };
    let mut impedances = rc_lowpass::Impedances::default();
    rc_lowpass::calc_impedances(&mut impedances, fs, params);
    let mut state = rc_lowpass::State::default();

    let mut max_error : f32 = 0.0;
    for i in 0..ref_output.len() {
        let test_output = rc_lowpass::process(&mut state, &impedances, 1.0);
    ...
    #[cfg(RUN_BENCH)]
    {
        ...
        for _ in 0..N_ITER {
            ...
            for n in 0..M {
                data_out[n] = rc_lowpass::process(&mut state, &impedances, data_in[n]);
            }
