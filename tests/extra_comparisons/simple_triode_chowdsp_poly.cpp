...
struct Reference_WDF
{
    ...
    // Grid circuit
    chowdsp::wdf::ResistiveVoltageSource<float> Vin { 25.0e3f };

    // Cathode circuit
    chowdsp::wdf::Resistor<float> Rk { 200.0f };

    // Plate circuit
    chowdsp::wdf::ResistiveVoltageSource<float> Vplus { 47.0e3f };

    chowdsp::wdf::Resistor<float> Rl { 330.0e3f };
    chowdsp::wdf::Capacitor<float> Cp { 0.047e-6f };
    chowdsp::wdf::WDFSeries<float> Sp { &Rl, &Cp };

    chowdsp::wdf::WDFParallel<float> Pp { &Vplus, &Sp };
...
int main()
{
    std::cout << "Simple Triode test (chowdsp_wdf polymorphic)\n";
